...
module.exports = {
    ...
    addCase: function (guild) {
        let r = this.getDB();
        r.db("modbot_db").table("cases").filter({
            guildId: guild.id
        }).run().then(row => {
    ...
    getCase: async function (guild) {
            let r = this.getDB();
            let data = await r.db("modbot_db").table("cases").filter({
                guildId: guild.id
            }).run();
            if (!data[0]) return 0;
        ...
        getPrefix: async function (guild) {
                let r = this.getDB();
                let data = await r.db("modbot_db").table("prefixes").filter({
                    guildId: guild.id
                }).run();
            ...
            setPrefix: function (guild, prefix) {
                let r = this.getDB();
                r.db("modbot_db").table("prefixes").filter({
                    guildId: guild.id
                }).run().then(row => {
                    let data = row[0];
                    if (!data) r.db("modbot_db").table("prefixes").insert({
                        guildId: guild.id,
                        prefix: prefix
                    }).run();
                    else r.db("modbot_db").table("prefixes").update({
                        prefix: prefix
                    }).run();
                });
            },
            ...
            getUserPrefix: async function (user) {
                    let r = this.getDB();
                    let data = await r.db("modbot_db").table("userprefixes").filter({
                        userId: user.id
                    }).run();
                ...
                setUserPrefix: function (user, prefix) {
                    let r = this.getDB();
                    r.db("modbot_db").table("userprefixes").filter({
                        userId: user.id
                    }).run().then(row => {
                        let data = row[0];
                        if (!data) r.db("modbot_db").table("userprefixes").insert({
                            userId: user.id,
                            prefix: prefix
                        }).run();
                        if (!data) r.db("modbot_db").table("userprefixes").filter({
                            userId: user.id
                        }).update({
                            prefix: prefix
                        }).run();
                ...
                channelIsBlocked: async function (channel) {
                        let r = this.getDB();
                        let data = await r.db("modbot_db").table("ignoredchannels").filter({
                    ...
                    triggeredMaxMentions: async function (msg) {
                            let r = this.getDB();
                            let data = await r.db("modbot_db").table("maxmentions").filter({
                        ...
                        triggeredAntiInvite: async function (msg) {
                            ...
                            handleMessage: async function (bot, msg) {
                                    ...
                                    if (await this.triggeredAntiInvite(msg)) {
                                        if (msg.member.permission.has('ADMINISTRATOR')) return;
                                        r.db("modbot_db").table("antiinvite").filter({
                                            guildId: msg.channel.guild.id
                                        }).run().then(row => {
                                            if (!row[0]) return;
                                            let botmember = msg.channel.guild.members.get(bot.user.id);
                                ...
                                handleXP: async function (msg) {
                                        let r = this.getDB();
                                        if (msg.author.bot) return;
                                        var foo = await r.db("modbot_db").table("xp").filter({
                                            guildId: `${msg.channel.guild.id}`
                                        }).run();
                                        if (!foo[0]) return;
                                        ...
                                        if (!olduserdata) {
                                            ...
                                            r.db("modbot_db").table("xp").filter({
                                                guildId: msg.channel.guild.id
                                            }).update({
                                                users: users
                                            }).run();
                                    ...
                                    antiSpam: async function (msg) {
                                            ...
                                            if (msg.author.bot) return;
                                            if (msg.member.permission.has("ADMINISTRATOR")) return;
                                            var foo = await r.db("modbot_db").table("antispam").filter({
                                            ...
                                            r.db("modbot_db").table("antispam").filter({
                                                channelId: msg.channel.id
                                            }).run().then(row => {
                                                if (row[0].users) {
                                                    if (!row[0].users[`${user}`]) {
